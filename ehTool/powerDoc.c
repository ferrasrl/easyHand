...
//	CHAR *lpText;
	...
	switch (enCmd)
	{
		...
		case WS_CREATE:
			...
			_sPower.colLineHorzColor=pwdGray(.6);//RGB(128,128,128));
	...
	*/			
	...
	//-----------------------------------------------------------------------
		...
		case WS_SETFLAG:
			...
			psField->colTitBack=pwdGray(.6);
			psField->colTitText=PDC_WHITE;
...
//				_sPower.arsField=ehAlloc(_sPd.lstElement->iLength*_sPd.lstElement->iSize);
				/*
				if (!_sPd.fArray) ehExit("LR6"); 
				_sPd.fArray=FALSE;
				_sPd.hmField=ARMaker(WS_CLOSE,"*arField");
				_sPower.arsField=memoPtr(_sPd.hmField,NULL);
				// Conto i campi 
				for (a=0;;a++)
				{
					if (psField==NULL) break;
				//win_infoarg("%d) [%s]",a,psField->szTitolo);
				}
				_sPower.iFieldNum=a;
				*/
...
//				if (_sPower.umRowLastHeight>_sPower.umRowMaxHeight) _sPower.umRowMaxHeight=_sPower.umRowLastHeight;
			...
			if (_sPower.fLineHorzField)
			{
				pwdLine(pwdRectFill(&rumChar,_sPower.rumPage.left,_sPower.umRowCursor,_sPower.rumPage.right,_sPower.umRowCursor),
						_sPower.colLineHorzColor,//arField[0]->colText,
			...
			if (_sPower.fLineVertField&&_sPower.iLinePerRiga>1)
			{
				...
				for (a=0;a<_sPower.iFieldNum;a++)
				{
					...
					_(sBox);
					sBox.sPenBrush.colPen.ehColor=0; _addItem(PDT_RECT,&rumRect,&sBox,sizeof(sBox));
...
void pwdSetEx(INT iCol,void * pValue,PWD_COLOR colText,PWD_COLOR colBack,EH_TSTYLE enStyles,PWD_ALIGN enAlign) {
	...
	//
	if (psFld->fCharClean) pszText=strTrim(strEver(pszText));
...
static void _LPreviewSizeCalc(void)
{
	SIZE sizMaxPage;
	...
	//
	// Determino la grandella fisica della pagina visible di preview
	//
			...
			if (_sPower.fLineVertField)
			{
				...
				if (!a)
				{
					...
					pwdLine(pwdRectFill(&rumLine,
						...
						rumRect.left,
						rumRect.bottom),
						psFld->colText,pwdUm(PUM_STD,1),PS_SOLID);
				}
				pwdLine(pwdRectFill(&rumLine,
					rumRect.right,
					rumRect.top,
...
static void _drawLayout(void)
{
	...
	case PWD_LAYTYPE1:
		...
		pwdLine(&rumRect,PDC_BLACK,pwdUm(PUM_STD,1),PS_SOLID);
...
//	if (psBox->sPenBrush.colBrush!=EH_COLOR_TRASPARENT) 	{
	...
	if (sizRound.cx||sizRound.cy) 
		RoundRect(psDraw->hDC,psDraw->recObj.left,psDraw->recObj.top,psDraw->recObj.right,psDraw->recObj.bottom,sizRound.cx,sizRound.cy);
		else
		Rectangle(psDraw->hDC,psDraw->recObj.left,psDraw->recObj.top,psDraw->recObj.right,psDraw->recObj.bottom);
...
static void _drawLine(PWD_DRAW *psDraw) {
	...
	MoveToEx(psDraw->hDC,psDraw->recObj.left,psDraw->recObj.top,NULL); // Cambiare NULL
	LineTo(psDraw->hDC,psDraw->recObj.right,psDraw->recObj.bottom); 
		...
		switch (psEle->enType) {
			...
			case PHT_MOVETO:
				_pointToPixel(	pumPoint->x,
								...
								&iX,
								&iY);
				MoveToEx(psDraw->hDC,iX,iY,NULL); // Cambiare NULL
				break;
			...
			case PHT_LINETO:
				_pointToPixel(	pumPoint->x,
								...
								&iX,
								&iY);
				LineTo(psDraw->hDC,iX,iY); // Cambiare NULL
				break;
...
//	INT x,y,xDim,yDim;
	...
	sizeCalc(&sizImage,&psDraw->recObj);
	...
	switch (psDraw->psItem->enType)
	{
		...
		case PDT_EHIMG:  
				if (psImage->hImage) 
					_ImgOutEx(psDraw->hDC, psDraw->recObj.left, psDraw->recObj.top, sizImage.cx, sizImage.cy,psImage->hImage);
				else
				{
					//
					...
					dcBmpDisp(	psDraw->hDC,
								psDraw->recObj.left, psDraw->recObj.top,
			...
			if (ImageList_GetImageInfo(psImage->himl,psImage->hImage,&sImageInfo))
				{
					...
					_ImgOutEx(psDraw->hDC,psDraw->recObj.left, psDraw->recObj.top, sizImage.cx, sizImage.cy,hdlImage);
			   ...
			   DWORD iColorDeepBit) // 0=Non ricodifico 1/4/8/24
{
	...
	if (!bStore) {
		...
		_LImageResampleSave(hImageTemp, sizImageSorg, sizImageDest,szFileTemp);
		sImage.pszFileTemp=strDup(szFileTemp);
...
/*
static void _TextSetFont(HPDF_Page psPage,HPDF_Font psFont,PWD_VAL dFontAlt,HPDF_REAL *pdCharOffset) {
...
static void _TextSetFont(HPDF_Page psPage,HPDF_Font psFont,PWD_VAL dFontAlt,HPDF_REAL * pdBaseLineOffset) {
...
//								rumArea.left=rumArea.right-drValue;
						...
						case PDA_CENTER:
								//rumArea.right+=drValue/2;
								//rumArea.left+=(sumArea.cx/2)-(drValue/2);
					...
					if (psText->enStyles&STYLE_UNDERLINE) {
						...
						_pdfLine(psPage,&pdfRect,psText->colText,dLine);
					}
					ehFree(pszText);
					break;
...
//				dwPos=((DWORD) sPthe.psData)+dwBezier;
...
*/
		...
		default:
			printf("metaType ? %d" ,pEmfRecord->iType);
			break;
