...
static const DWORD _orig_p[16 + 2] = {
...
static const DWORD _orig_s[4][256] = {
...
static DWORD _fx(S_BLOWFISH *psCtx, DWORD x) {
   BYTE a, b, c, d;
   DWORD  y;
...
// bfEncrypt()
//
void bfEncrypt(S_BLOWFISH * psCtx, DWORD *xl, DWORD *xr){
...
// bfDecrypt()
//
void bfDecrypt(S_BLOWFISH *psCtx, DWORD *xl, DWORD *xr) {
...
// bfInit()
//
void bfInit(S_BLOWFISH *psCtx, unsigned char *key, int keyLen) {
  ...
  for (i = 0; i < NDEEP + 2; ++i) {
    ...
    psCtx->P[i] = _orig_p[i] ^ data;
  }

  datal = 0x00000000;
  datar = 0x00000000;

  for (i = 0; i < NDEEP + 2; i += 2) {
    bfEncrypt(psCtx, &datal, &datar);
    psCtx->P[i] = datal;
    psCtx->P[i + 1] = datar;
  }
...
// 
CHAR * bfHexDecrypt(CHAR * pszCryptHex,
					CHAR * pszKey,
					CHAR * pszDest,
					INT iSizeDest) {
	...
	for (p=pszCryptHex;*p;) {
		...
		if (strlen(p)<8) {bError=true; break;} // errore
		memcpy(szBuf,p,8); szBuf[8]=0; 
		dwL=xtoi(szBuf); p+=8;

		if (strlen(p)<8) {bError=true; break;} // errore
		memcpy(szBuf,p,8); szBuf[8]=0; p+=8;
		dwR=xtoi(szBuf);
...
CHAR * bfHexDecryptAlloc(CHAR * pszCryptHex,CHAR * pszKey) {
	...
	INT		iSize=strlen(pszCryptHex)*10;
	CHAR *	pszRet=ehAlloc(iSize+1);
	if (!bfHexDecrypt(pszCryptHex,pszKey,pszRet,iSize)) ehFreePtr(&pszRet);
	return pszRet;
}


	/*
	Blowfish_Encrypt(&psCtx, &L, &R);
	printf("%08lX %08lX\n", L, R);
	if (L == 0xDF333FD2L && R == 0x30A71BB4L)
	  printf("Test encryption OK.\n");
	else
	  printf("Test encryption failed.\n");
	Blowfish_Decrypt(&psCtx, &L, &R);
	if (L == 1 && R == 2)
...
// bfHexEncrypt()
//
CHAR * bfHexEncrypt(CHAR * pszSource,
					CHAR * pszKey,
					CHAR * pszHexDest,
					INT iHexDest) {
	...
	*pszHexDest=0; c=0;
	...
	if (bError) {*pszHexDest=0; return NULL;} else return pszHexDest;
...
CHAR * bfHexEncryptAlloc(CHAR * pszCryptHex,CHAR * pszKey) {
	...
	INT		iSize=strlen(pszCryptHex)*8;
	CHAR *	pszRet=ehAlloc(iSize+1);
