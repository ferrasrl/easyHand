...
Usa component IWebBrowser2

This is a Win32 C application (ie, no MFC, WTL, nor even any C++ -- just plain C) that 
demonstrates how to embed a browser "control" (actually, an OLE object) in your own window 
(in order to display a web page, or an HTML file on disk).

This is very loosely based upon a C++ example written by Chris Becke. I used that to learn 
the minimum of what I needed to know about hosting the browser object. 
Then I wrote this example from the ground up in C.

*/
#define CINTERFACE
#include "/easyhand/inc/easyhand.h"
#include "/easyhand/ehtool/ZCMP_WebPage.h"
...
static IDispatch *	_getItemInterface(EH_WEBPAGE * psWebPage,CHAR *pszInputName,BSTR * pbstrName);
...
void * ehzWebPage(EH_OBJ *objCalled,EN_MESSAGE cmd,LONG info,void *ptr)
{
	...
	switch(cmd)	{
		...
		case WS_REALSET:
			if (strstr(ptr,".")) {_FormSetField(psWebPage,info,ptr); return NULL;}
...
static void _localAddFree(BYTE *lpType,BYTE *lpTagName,BYTE *lpValue,S_ARMAKER *lps,BOOL fChecked)
{
	BYTE *lpString;
	BOOL bTake=TRUE;
	if (lpTagName&&lpType)
...
void BeforeNavigate2(EH_WEBPAGE *psWebPage,IDispatch *pDisp, VARIANT * url,VARIANT * Flags,VARIANT * TargetFrameName, VARIANT * PostData,VARIANT * Headers,  VARIANT_BOOL * Cancel) 
{
...
//
//  _getItemInterface() - Ritorna il Interface Dispact di un item
//
static IDispatch * _getItemInterface(EH_WEBPAGE * psWebPage,CHAR *pszInputName,BSTR * pbstrName) {
	...
	IHTMLElementCollection * piElemInput=NULL;
	IHTMLElement * piTag=NULL;
	_StrToBstr(psWebPage, pszInputName, &bsName);
	hr=psWebPage->sIe.piHtmlDoc3->lpVtbl->getElementsByName(psWebPage->sIe.piHtmlDoc3,bsName,&piElemInput);
	*pbstrName=NULL;
	//
	// Recupero il singolo Tag all'indice "varIndex" da pElemFields -> pTagDisp
	//
	if ( (SUCCEEDED(hr)) && (piElemInput!=NULL) ) 
	{
		VARIANT varIndex;
		VariantInit(&varIndex);
		varIndex.vt = VT_I4;
		varIndex.lVal = 0;
		hr = piElemInput->lpVtbl->item( piElemInput, varIndex, varIndex, &piItemDispatch); if (hr) ehError();
		VariantClear(&varIndex);
		if (piItemDispatch) {
			hr = piItemDispatch->lpVtbl->QueryInterface(piItemDispatch, &IID_IHTMLElement, (void**) &piTag );
			if ((SUCCEEDED(hr)) && (piTag!=NULL) ) {
				piTag->lpVtbl->get_tagName(piTag,pbstrName); 
				_$Release(piTag);
			}	
...
static CHAR * _JavaLikeGetValue(EH_WEBPAGE *psWebPage,SINT iMode,CHAR *pszFormInputName)
{
	...
	piItemDispatch=_getItemInterface(psWebPage,pszInputName,&bsName);
	if (piItemDispatch) 
	{
		fFound=TRUE;
		//
		...
		if (!_wcsicmp(bsName,L"INPUT")) {
			...
			if ( (SUCCEEDED(hr)) && piInput ){
				...
				if (bstrType) {
					...
					if (!_wcsicmp(bstrType,L"checkbox")||!_wcsicmp(bstrType,L"radio")) {
						fChecked=FALSE; piInput->lpVtbl->get_checked(piInput,&fChecked); 
						if (fChecked) piInput->lpVtbl->get_value(piInput,&bstrReturn);
					}
					else if (!_wcsicmp(bstrType,L"text")||!_wcsicmp(bstrType,L"hidden")) 
		...
		// TEXTAREA
		//
		else if (!_wcsicmp(bsName,L"TEXTAREA")) {
		...
		// SELECT
		//
		else if (!_wcsicmp(bsName,L"SELECT")) {
...
static BOOL	_JavaLikeSetValue(EH_WEBPAGE * psWebPage,BYTE * pszInputName,void * pszInputValue) {
	...
	if (piItemDispatch) 
	{
		...
		//
		else if (!_wcsicmp(bsName,L"INPUT"))
		{
			IHTMLInputElement* piInput = NULL;
			short fChecked;
			hr = piItemDispatch->lpVtbl->QueryInterface(piItemDispatch, &IID_IHTMLInputElement, (void**) &piInput );
			if ( SUCCEEDED(hr) && piInput )
			{
				...
				if (bsType) {
					...
					if (!_wcsicmp(bsType,L"checkbox")||
						!_wcsicmp(bsType,L"radio"))
					{
						fChecked=TRUE;
						piInput->lpVtbl->put_checked(piInput,fChecked); 
					}

					if (!_wcsicmp(bsType,L"text")||
						!_wcsicmp(bsType,L"hidden"))
					{
...
static SINT _formArrayMaker(HWND hwnd,CHAR *lpFormName)
{
	...
	{
		if (psWebPage->sIe.iTags>0) ARMakerEx(WS_OPEN,NULL,&sArMaker);
				...
				if ( (SUCCEEDED(hr)) && (piTag!=NULL) )
				{
					...
					if (!strcmp(lpName,"INPUT"))
					{
						...
						if ( (SUCCEEDED(hr)) && piInput )
						{
							...
							_localAddFree(lpType,lpTagName,lpValue,&sArMaker,fChecked);
					...
					if (!strcmp(lpName,"TEXTAREA")) {
						...
						if ( SUCCEEDED(hr) && piTextArea )
						{
							...
							_localAddFree(lpType,lpTagName,lpValue,&sArMaker,TRUE);
							_$Release(piTextArea);
						}
					}
...
BOOL WPCallJScript(CHAR *lpObjName,CHAR *lpFunction,CHAR *lpResult,SINT iResultBufferSize,SINT iArgs,...) {
	...
	OLECHAR FAR* olcFunction;// = L"Update";
	wchar_t *lpwFunction;
	va_start(Arg,iArgs);

	//iIndex=WPFind(WP_FINDOBJN,lpObjName);
	psObj=obj_GetInfo(lpObjName);  if (!psObj) ehExit("WPCallJScript([%s]) ?",lpObjName);
	psWebPage=psObj->pOther; if (!psWebPage) ehExit("WPCallJScript 2 ([%s]) ?",lpObjName);
	lpwFunction=strToWcs(lpFunction);
	olcFunction=lpwFunction;
	...
	{      
		...
		if (!_ieCreate(psWebPage,FALSE))//,psWebPage->hWnd,FALSE))//&psWebPage->browserObject, &webBrowser2, &lpDispatch, &htmlDoc2))
		{
			...
			LPDISPATCH spScript=NULL;
			VARIANT vaResult;

			// Cerco script del documento
			hr=psWebPage->sIe.piHtmlDoc2->lpVtbl->get_Script(psWebPage->sIe.piHtmlDoc2,&spScript);
			if (!SUCCEEDED(hr) || !spScript)
			{
				win_infoarg("get_Script() on function %s",lpFunction);
	//			goto FINE;
			}
			else
			{
			 //_StrToBstr(lpFunction, &bstrFunc);
			 hr = spScript->lpVtbl->GetIDsOfNames(spScript,
												 &IID_NULL,
												 &olcFunction,//bstrFunc,
												 1,
			 ...
			 if(FAILED(hr))
			 {
				 ...
				 osError(FALSE,hr,"WPCallJScript(): Non trovo funzione %s() obj:%s",lpFunction,lpObjName);
			//	goto FINE2;
			 }
			 else
			 {

				// Preparazione degli argomenti da passare alla funzione
				ZeroFill(dispparams);
				dispparams.cArgs = iArgs; // Numero dei parametri
				if (dispparams.cArgs>0) dispparams.rgvarg=ehAlloc(sizeof(VARIANT)*dispparams.cArgs);
				...
				for(i=0;i< iArgs; i++)
				{
					...
					pt=va_arg(Arg,CHAR *); // win_infoarg("[%s]",pt);
					_StrToBstr(psWebPage, pt, &bstrArg);
					dispparams.rgvarg[iArgs-i-1].vt = VT_BSTR;
					dispparams.rgvarg[iArgs-i-1].bstrVal=bstrArg;
				}
				...
				hr = spScript->lpVtbl->Invoke(spScript,
											dispid,
											&IID_NULL,
											0,
											DISPATCH_METHOD,
				...
				if (dispparams.rgvarg) 
				{
					...
					ehFree(dispparams.rgvarg);
				  }			  if (FAILED(hr)) 
				  {
					  //GetSystemErrorMessage(hr);
					  osError(FALSE,0,"Invoke Error [%s]",lpFunction);
				  }	
				 }
			}
			if (spScript) spScript->lpVtbl->Release(spScript);
		}

	}__except (ehLogWrite("Errorex"), 1) {
			ehLogWrite("Exception %s.%s",lpObjName,lpFunction);   
	}  
...
BOOL WPSetVar(CHAR *pObjName,CHAR *pFormInput,CHAR *pArgs,...)
{
	...
	{
		va_start(Ah,pArgs);
		sprintf(lpBuf,"%s=",pFormInput);
		vsprintf(strNext(lpBuf),pArgs,Ah); 
		obj_message(pObjName,WS_REALSET,0,lpBuf); 
		va_end(Ah);
	}
	ehFree(lpBuf);
	return FALSE;
...
static void _FormSetField(EH_WEBPAGE *psWebPage,SINT iMode,CHAR *lpFormName) {
	...
	strcpy(szForm,p); 
	ehFree(p);
	pszInputName=strExtract(lpFormName,".","=",FALSE,FALSE); if (!pszInputName) ehExit("_FormSetField: [%s] indicare il campo da cercare",pszInputName);
	pszInputValue=strstr(lpFormName,"="); if (!pszInputValue) ehExit("_JavaLikeGetValue: [%s] indicare il valore",pszInputName);
	pszInputValue++;
...
CHAR * WPGetVar(CHAR *pObjName,CHAR *pFormInput)  {
	return obj_message(pObjName,WS_REALGET,0,pFormInput); 
}

CHAR * WPEventFunction(void)
{
	return "function putEvent(TipoEvento)" 
			"{" 
				"var v='easyhand:'+TipoEvento;" CRLF //+'tag:'+e.tagName+';'+'id:'+e.id+';';" CRLF
				"window.location.href=v;" CRLF
			"};";
}
...
BOOL WPObjectReady(CHAR *lpObjName,CHAR *lpFunction,SINT iTimeOutSec)
{
	...
	//iIndex=WPFind(WP_FINDOBJN,lpObjName);
	psObj=obj_GetInfo(lpObjName);  if (!psObj) ehExit("WPObjectReady([%s]) ?",lpObjName);
	psWebPage=psObj->pOther; if (!psWebPage) ehExit("WPObjectReady 2 ([%s]) ?",lpObjName);
	lpwFunction=strToWcs(lpFunction);
	olcFunction=lpwFunction;
	...
	while (TRUE)
	{
		tDiff=(clock()-tStart)/1000; if (tDiff>iTimeOutSec) break; // Timeout
		WindowsMessageDispatch();
		__try
		{      
			//
			// Check delle interfacce
			//
			if (!_ieCreate(psWebPage,FALSE))//,psWebPage->hWnd,FALSE))//&psWebPage->browserObject, &webBrowser2, &lpDispatch, &htmlDoc2))
				{
					LPDISPATCH spScript=NULL;
					...
					// Cerco script del documento
					hr=psWebPage->sIe.piHtmlDoc2->lpVtbl->get_Script(psWebPage->sIe.piHtmlDoc2,&spScript);
					if (!SUCCEEDED(hr) || !spScript)  {win_infoarg("get_Script() on function %s",lpFunction); }
					else
					{
					 //_StrToBstr(lpFunction, &bstrFunc);
					 hr = spScript->lpVtbl->GetIDsOfNames(spScript,
														 ...
														 &dispid);
					  if (spScript) spScript->lpVtbl->Release(spScript);
					  if (!FAILED(hr)) {bError=FALSE; break; }// <-- Esco se funziona
					}
				}
		}
		__except (win_infoarg("Errore"), 1) {
			win_infoarg("Exception %s.%s",lpObjName,lpFunction);   
		}  
	}
	return bError;
}
